...
function CharactersPage() {
  return (
    <PageContainer>
      <SectionTitle eyebrow="A Caravana" title="Personagens" subtitle="Os que ainda caminham, e o que carregam." />
          ...
          <li key={c.slug}>
            <Link to="/personagens/$slug" params={{ slug: c.slug }} className="block">
              <ChronicleCard as="article" className="h-full">
                ...
                <p className="mt-1 text-xs uppercase tracking-widest text-muted-foreground">{c.people}</p>
                <p className="mt-3 italic text-foreground/85">“{c.quote}”</p>
                <p className="mt-4 text-xs uppercase tracking-[0.25em] text-[var(--gold)]/80">Ver personagem →</p>
              </ChronicleCard>
            </Link>
          </li>
        ))}
