      ...
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-4 lg:gap-5">
          <Link to="/" className="group flex min-w-0 items-center gap-2.5">
            <span
              aria-hidden
              className="inline-block h-7 w-7 shrink-0 rounded-full border border-[var(--gold)]/60 bg-[radial-gradient(circle,_var(--gold)_0%,_transparent_65%)] opacity-80 shadow-[0_0_18px_oklch(0.72_0.12_80_/_0.18)]"
            />
            <span className="truncate font-display text-xl tracking-[0.16em] text-[var(--gold)]">
              TENEBRE
            </span>
          </Link>

          <nav aria-label="Navegação principal" className="hidden md:block">
            <ul className="flex items-center gap-0.5 border-l border-border/70 pl-4 lg:pl-5">
              {nav.map((item) => {
                ...
                return (
                  <li key={item.to}>
                    <Link
                      to={item.to}
                      className={`inline-flex h-9 items-center rounded px-2.5 text-sm tracking-wide transition-colors hover:bg-accent/35 hover:text-[var(--gold)] lg:px-3 ${
